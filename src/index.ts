import { exportGlobal } from './utils'
...
function findAllChapters(): Chapter[] {
...
exportGlobal('findAllChapters', findAllChapters)

function getKcdm() {
  return document.querySelector<HTMLInputElement>('input#kcdm').value
}

async function findVideosOfChapter(chapter: Chapter): Promise<Video[]> {
  let kcdm = getKcdm()
...
exportGlobal('findVideosOfChapter', findVideosOfChapter)